    ...
    <header className="sticky top-0 z-40 border-b border-white/70 bg-white/75 backdrop-blur-xl">
      <div className="container-page py-3">
        <div className="flex items-center justify-between gap-4 rounded-[28px] border border-white/60 bg-white/70 px-4 py-3 shadow-soft">
          <BrandLogo to={isAdmin ? '/admin' : '/'} title="BentoBox" subtitle="Northern University Bangladesh" />
          ...
          <nav className="hidden items-center gap-2 lg:flex">
            ...
            {isAdmin ? (
              <Link to="/" className="rounded-full px-4 py-2 text-sm font-medium text-slate-500 hover:bg-white hover:text-slate-950">
                Public site
              </Link>
            ) : null}
          </nav>
            ...
            <div className="flex flex-col gap-2">
              {navItems.map((item) => (
                <NavLink
                  ...
                  className={({ isActive }) => isActive ? 'rounded-2xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white' : 'rounded-2xl px-4 py-3 text-sm font-medium text-slate-700 hover:bg-slate-100'}
              ...
              {isAdmin ? (
                <Link to="/" onClick={() => setMenuOpen(false)} className="rounded-2xl px-4 py-3 text-sm font-medium text-slate-700 hover:bg-slate-100">
                  Public site
                </Link>
              ) : null}
              ...
              {isAuthenticated ? (
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <Link to="/register" onClick={() => setMenuOpen(false)} className="btn-primary w-full">Create account</Link>
                </div>
              )}
            </div>
          </div>
